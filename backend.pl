...
$table2 = "CREATE TABLE IF NOT EXISTS deviceinfo (
...
$query2->execute;
my $know = $dbh->prepare("SELECT * FROM deviceinfo WHERE ip = '$ip' && community ='$community' "); 
...
my @row1 = $know->fetchrow_array();
 ...
 if($row1[1] eq "$ip" && $row1[2] eq "$port" && $row1[3] eq "$community")
  ...
  {
 $req=$row1[4]+1;
  $update = $dbh->prepare("UPDATE deviceinfo SET  reqsent = '$req',  sysup ='$sysup',  systemtime='$updatetime', updatetime = '$updatetime' WHERE ip = '$ip' && community='$community'" );
  $update->execute;
   }

 else
   {
$insert = $dbh->prepare("INSERT INTO deviceinfo (ip, port, community, reqsent, lostreq, sysup, systemtime, updatetime) VALUES ('$ip' , '$port', '$community', '1','0','$sysup', '$updatetime', '$updatetime')");
$insert->execute;
    }
...
$query3->execute;
my $know2 = $dbh->prepare("SELECT * FROM deviceinfo WHERE  ip = '$ip' && community = '$community'" );
$know2->execute;
...
#no need to update ip port community as they are alredy present while verfiying logic
     $update2=$dbh->prepare("UPDATE deviceinfo SET reqsent = '$reqf', lostreq = '$lost', updatetime = '$updatetime', lastmissed = '$updatetime' WHERE ip= '$ip' && community='$community' ");
    $update2->execute;  
     }
else
    {
    $insert1 = $dbh->prepare("INSERT INTO deviceinfo (ip, port , community , reqsent ,lostreq , updatetime , lastmissed) VALUES ( '$ip', '$port' , '$community', '1' ,'1' ,'$updatetime' , '$updatetime')");
 $insert1->execute;
    }
